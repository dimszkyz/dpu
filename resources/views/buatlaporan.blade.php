...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    @if ($errors->any())
        <div class="p-4 bg-red-50 border border-red-200 text-red-800 text-xs font-semibold rounded-xl space-y-1 shadow-sm">
            <div class="font-bold text-red-700 mb-1">Form belum lengkap:</div>
        ...
        @if(!$is_waktu_habis)
            <form action="{{ route('laporan.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 sm:p-8 space-y-6">
                        ...
                        <div>
                            ...
                            @if($deadline)
                                <p class="text-sm font-medium text-slate-500 mt-1">
                                    Batas Waktu:
                                    <span class="text-indigo-600">{{ \Carbon\Carbon::parse($deadline)->format('d M Y, H:i') }}</span>
                                </p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        <label for="judul" class="block text-sm font-bold text-slate-700">Judul Laporan <span class="text-slate-400 text-xs">(Opsional)</span></label>
                        <input type="text" name="judul" id="judul" value="{{ old('judul') }}"
                            ...
                            placeholder="Contoh: Laporan Penyelesaian Survei Lapangan">
                    </div>

                    <div class="space-y-1">
                        <label for="teks_laporan" class="block text-sm font-bold text-slate-700">Deskripsi / Isi Laporan <span class="text-red-500">*</span></label>
                        <textarea name="teks_laporan" id="teks_laporan" rows="6" required
                            ...
                            placeholder="Jelaskan secara detail hasil pekerjaan yang telah Anda selesaikan...">{{ old('teks_laporan') }}</textarea>
                        @error('teks_laporan')
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-bold text-slate-700">Lampiran Berkas <span class="text-slate-400 text-xs">(Opsional)</span></label>
                        <div class="relative group">
                            <input type="file" name="file_laporan[]" id="files" multiple
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.zip">
                            <div class="w-full px-6 py-8 border-2 border-dashed border-slate-200/80 rounded-xl hover:border-blue-400 hover:shadow-sm transition-all group">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="p-2 bg-blue-50 text-blue-600 rounded-lg shrink-0 font-mono text-[10px] font-black uppercase tracking-wider">BERKAS</div>
                                    <div class="min-w-0">
                                        <p class="text-xs font-bold text-slate-800 truncate group-hover:text-blue-600 transition-colors">Pilih Berkas Lampiran</p>
        ...
        @else
            <div class="p-12 text-center space-y-4">
                ...
                <p class="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                    Anda tidak dapat mengirimkan laporan baru karena batas waktu pengerjaan tugas telah terlampaui. Silakan hubungi admin untuk mengajukan perpanjangan batas lapor.
                </p>
